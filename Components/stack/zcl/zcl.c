...
uint16 zcl_event_loop(uint8 task_id, uint16 events)
{
    ...
    if(events & SYS_EVENT_MSG)
    {
        ...
        //debug_str("zcl_event_loop in zcl.c");

        while(msgPtr != NULL)
        {
            uint8 dealloc = TRUE;
            //cust_debug_str("zclloop %d",  *msgPtr);
            if(*msgPtr == AF_INCOMING_MSG_CMD)
            {
                //debug_str("zaimc");
                zcl_ProcessMessageMSG((afIncomingMSGPacket_t *)msgPtr);
                //cust_debug_str("zcl_Pstatus:%d", status);
...
static uint8 zcl_getExternalFoundationHandler(afIncomingMSGPacket_t *pInMsg)
{
    ...
    //cust_debug_str("zcl_getExternalFoundationHandler, endpoint:%d",
                   //pInMsg->endPoint);
...
zclProcMsgStatus_t zcl_ProcessMessageMSG(afIncomingMSGPacket_t *pkt)
{
    ...
    //cust_debug_str("ep is %d", epDesc->endPoint);
    
    if(epDesc == NULL)
    {
        debug_str("Can't find the EP");
        rawAFMsg = NULL;
    ...
    {
        debug_str("ZCL_PROC_NOT_OPERATIONAL");
        rawAFMsg = NULL;
    ...
    if((pInPlugin != NULL) && !zcl_DefaultRspCmd(inMsg.hdr))
    {
        ...
        //debug_str("It's not securityEnable");
    }

    // Is this a foundation type message
    //cust_debug_str("%d", inMsg.hdr.fc.type);
    if(!interPanMsg && zcl_ProfileCmd(inMsg.hdr.fc.type))
    {
        //debug_str("Its a foundation type message");
                ...
                (zclCmdTable[inMsg.hdr.commandID].pfnParseInProfile != NULL))
        {
            ...
            {
                //cust_debug_str("parse the command %d", inMsg.hdr.commandID);
                // Process the command
                if(zclProcessCmd(inMsg.hdr.commandID, &inMsg) == FALSE)
                {
                    debug_str("Couldn't find attribute in the table.");
    ...
    {
        debug_str("It's a cluster ID message");
    ...
    {
        //debug_str("send out a default response");
...
static uint8 zclProcessInReadCmd(zclIncoming_t *pInMsg)
{
    ...
    cust_debug_str("zclProcessInReadCmd");
    ...
    cust_debug_str("ep%d sadr%d cluster%d  direction%d", 
                                    pInMsg->msg->endPoint,
                                    0,
